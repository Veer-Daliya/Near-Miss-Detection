...
def save_yolo_detection_incremental(output_dir: str, image_name: str, detections: list) -> None:
    """
...
def save_ocr_detection_incremental(output_dir: str, image_name: str, ocr_results: list) -> None:
    """
...
def detect_all_text_regions(image: np.ndarray, ocr, min_confidence: float = 0.0):
    ...
    try:
        ...
        if hasattr(ocr, 'reader'):
            ...
            ocr_result = results[0] if isinstance(results, list) and len(results) > 0 else results
            ...
            if isinstance(ocr_result, dict):
                ...
                for i, (text, score, poly) in enumerate(zip(rec_texts, rec_scores, rec_polys)):
                    ...
                    try:
                        ...
                        if poly_array.ndim == 2 and poly_array.shape[1] == 2:
                            bbox_points = [[int(poly_array[j][0]), int(poly_array[j][1])] for j in range(poly_array.shape[0])]
                        else:
                            continue
                ...
                print(f"DEBUG: ocr_result is list (old format), count: {len(text_lines)}")
            else:
        ...
        for line in text_lines:
            ...
            if isinstance(bbox_points, list) and len(bbox_points) > 0:
                try:
                    roi_x1, roi_y1, roi_x2, roi_y2 = _standardize_bbox_from_polygon(bbox_points)
                except Exception as e:
                    print(f"Warning: Could not parse bbox_points: {bbox_points}, error: {e}")
                    continue
...
def process_image(
    ...
    for i, vehicle in enumerate(vehicles):
        print(f"  Vehicle {i+1}: {vehicle.class_name} (confidence: {vehicle.confidence:.2f})")
    ...
    for i, vehicle in enumerate(vehicles):
        ...
        # Detect text in ROI
        roi_text = detect_all_text_regions(vehicle_roi, ocr, min_confidence=plate_confidence_threshold)
        ...
        for j, det in enumerate(roi_text):
            ...
            print(f"      Size: {det['width']}x{det['height']}, Aspect: {det['aspect_ratio']:.2f}")
            ...
            cv2.rectangle(annotated, (frame_x1, frame_y1), (frame_x2, frame_y2), (255, 0, 0), 2)
            label = f"{det['text']} ({det['confidence']:.2f})"
            cv2.putText(annotated, label, (frame_x1, frame_y2 + 15), 
    ...
    if all_ocr_results:
        ...
        print(f"OCR detections saved to: {os.path.join(output_dir, 'ocr_bboxes.jsonl')}")
